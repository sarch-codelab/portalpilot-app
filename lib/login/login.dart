...
import 'package:portal_pilot_app/Areas/Educacion/educacion.dart'; // Importar educacion.dart
...
class _LoginScreenState extends State<LoginScreen>
  ...
  Future<void> _loadSavedData() async {
    ...
    _tenantController.text = prefs.getString('tenant_url') ?? '';
  ...
  Future<void> _handleLogin() async {
    ...
    try {
      ...
      if (tenantCode.isNotEmpty) {
        await prefs.setString('tenant_url', tenantCode);
      }
      await prefs.setString('saved_email', loggedUser.email);
      await prefs.setString('user_email', loggedUser.email);
      await prefs.setString('user_role', loggedUser.rol);
      await prefs.setString('user_area', loggedUser.area ?? '');
      await prefs.setString('company_code', loggedUser.empresaCodigo);
      await prefs.setString('user_nombre', loggedUser.nombre ?? '');
      await prefs.setString('user_apellido', loggedUser.apellido ?? '');
      ...
      Future.delayed(const Duration(milliseconds: 1500), () {
        ...
        if (area == 'educacion' || area == 'educación') {
          // Navegar directamente a educacion.dart (dashboard)
          ...
          // Root también va al dashboard de educación por ahora
          Navigator.pushReplacement(
            context,
            MaterialPageRoute(builder: (context) => const EducacionScreen()),
          );
          ...
          _notificationManager.showNotification(
            'Área corporativa no asignada. Contacta a tu Owner.',
    ...
    return Container(
      decoration: BoxDecoration(
        gradient: RadialGradient(
          ...
          radius: 1.4,
          colors: [
            accentPurpleDeep.withOpacity(0.25),
            accentPurpleDark.withOpacity(0.08),
            bgPrimary,
          ],
          ...
          child: ConstrainedBox(
            ...
            child: Column(
              ...
              children: [
                ...
                _buildStepTile(1, 'Conecta tu tenant', true),
  ...
  Widget _buildStepTile(int step, String label, bool active) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        borderRadius: BorderRadius.circular(16),
        border: Border.all(
          color: active ? accentPurple.withOpacity(0.4) : borderLight,
        ),
        boxShadow: active
            ? [
                BoxShadow(
      ...
      child: Column(
        ...
        children: [
          _buildInfoBanner(
            ...
            buttonText: 'Usar',
            onPressed: () => _notificationManager.showNotification(
          ...
          _buildInputField(
            ...
            validator: (v) =>
                (v == null || v.isEmpty) ? 'Ingresa la URL del tenant' : null,
          ),
          const SizedBox(height: 16),
          _buildInputField(
          ...
          Row(
            children: [
              Expanded(
                child: _buildSSOButton('Microsoft', Icons.desktop_windows_rounded),
              ),
              const SizedBox(width: 12),
              Expanded(
                child: _buildSSOButton('Google', Icons.g_mobiledata_rounded),
              ),
            ],
          ),
        ],
      ...
      child: Row(
        children: [
          Container(
            ...
            child: Icon(icon, color: accentPurple, size: 17),
          ),
          const SizedBox(width: 12),
          Expanded(
            child: Column(
  ...
  Widget _buildSSOButton(String label, IconData icon) {
    return Container(
      ...
      child: TextButton.icon(
        ...
        icon: Icon(icon, size: 18, color: textPrimary),
        label: Text(
          label,
          style: GoogleFonts.dmSans(
            fontSize: 13,
