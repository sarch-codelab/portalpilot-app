// lib/Areas/Educacion/IA/copilot_screen.dart
import 'dart:async';
...
import 'package:portal_pilot_app/Areas/Educacion/educacion.dart';
import 'package:portal_pilot_app/IA/ia.dart';
import 'package:portal_pilot_app/Areas/Educacion/Edu IA/reglas_ia.dart';
import 'package:portal_pilot_app/Areas/Educacion/Edu IA/document_processor.dart';
...
class _CopilotScreenState extends State<CopilotScreen> with TickerProviderStateMixin {
  ...
  Widget _buildAILogo({double size = 32}) {
    return ClipOval(
      child: Image.asset(
        'lib/Areas/Educacion/img/iconos/pp.edu.png',
        width: size,
        height: size,
        fit: BoxFit.cover,
            ...
            child: Container(
              decoration: BoxDecoration(
                gradient: RadialGradient(
                  ...
                  radius: 1.5,
                  colors: [
                    p.accentPurple.withOpacity(p.isDark ? 0.05 : 0.08),
                    p.bgPrimary,
                  ],
                ),
              ),
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          Text('¡Hola, ${_nombreUsuario.split(' ')[0]}! 👋',
    ...
    return Column(
      ...
      children: [
        Container(
          ...
          decoration: BoxDecoration(
            color: p.bgSecondary.withOpacity(0.5),
      ...
      children: [
        ...
        Container(
          decoration: BoxDecoration(
            ...
            boxShadow: [
              BoxShadow(
                color: Colors.black.withOpacity(p.isDark ? 0.2 : 0.05),
                blurRadius: 20,
  ...
  Widget _buildFileChip(AttachedFile file, ThemePalette p) {
    return Container(
      padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 6),
      decoration: BoxDecoration(
        color: file.color.withOpacity(0.12),
        borderRadius: BorderRadius.circular(8),
        border: Border.all(color: file.color.withOpacity(0.3)),
      ),
      child: Row(
        mainAxisSize: MainAxisSize.min,
        children: [
        ...
        child: Column(
          ...
          children: [
            ...
            Container(
              ...
              decoration: BoxDecoration(
                color: msg.isMe ? p.accentPurple.withOpacity(0.1) : p.cardColor,
                borderRadius: BorderRadius.circular(14),
                border: Border.all(
                    color: msg.isMe ? p.accentPurple.withOpacity(0.25) : p.borderLight),
              ),
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                      ...
                      return Container(
                        ...
                        child: Transform.translate(
                          ...
                          child: Container(
                            ...
                            height: 8,
                            decoration: BoxDecoration(
                              color: p.accentPurple.withOpacity(opacity),
                              shape: BoxShape.circle,
                              boxShadow: [
                                BoxShadow(
                                  color: p.accentPurple.withOpacity(opacity * 0.5),
                                  blurRadius: 6,
                                ),
                              ],
                            ),
      ...
      child: Material(
        ...
        child: InkWell(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              color: isActive ? p.accentPurple.withOpacity(0.1) : Colors.transparent,
              borderRadius: BorderRadius.circular(8),
            ),
            child: Row(
              children: [
      ...
      if (result != null && result.files.isNotEmpty) {
        setState(() {
          for (final file in result.files) {
            ...
            String type = 'other';
            if (ext == 'pdf') type = 'pdf';
            else if (['jpg', 'jpeg', 'png'].contains(ext)) type = 'image';
            else if (['doc', 'docx', 'txt'].contains(ext)) type = 'doc';
  ...
  void _enviarMensaje(String texto, {bool regenerate = false}) {
    ...
    _generarRespuestaReal(text).then((respuesta) {
      if (!mounted) return;
...
''';
    ...
    if (response.success) {
      ...
      return '⚠️ **Error al generar respuesta**\n\n${response.error}\n\nPor favor, intenta de nuevo.';
    }
  } catch (e) {
  ...
  try {
    ...
    final buffer = StringBuffer();
    buffer.writeln('## 📊 Base de Datos de Matrículas');
    buffer.writeln('**Empresa:** $_empresaCodigo');
...
String _contarPorCampo(List<Map<String, dynamic>> lista, String campo) {
  ...
  String _contarEstados(List<Map<String, dynamic>> matriculas) {
    final estados = <String, int>{};
    for (final m in matriculas) {
      final estado = m['estado'] ?? 'desconocido';
      estados[estado] = (estados[estado] ?? 0) + 1;
    }
    return estados.entries.map((e) => '${e.key}: ${e.value}').join(', ');
  }
