...
class _CreditoAvanzadoState extends State<CreditoAvanzado> {
  List<Map<String, dynamic>> _clientes = [
    {'id': '1', 'nombre': 'Pulpería Doña María', 'limite': 5000.00, 'usado': 2340.00, 'disponible': 2660.00, 'estado': 'activo'},
    {'id': '2', 'nombre': 'Mercadito San José', 'limite': 3500.00, 'usado': 3500.00, 'disponible': 0.00, 'estado': 'sobre_limite'},
    {'id': '3', 'nombre': 'Abarrotería El Vecino', 'limite': 2000.00, 'usado': 890.00, 'disponible': 1110.00, 'estado': 'activo'},
  ];
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text('Control de Crédito Avanzado', style: GoogleFonts.syne(fontSize: 15, fontWeight: FontWeight.w900, color: Colors.white, letterSpacing: 1.5)),
        actions: [
          IconButton(
            icon: Icon(
      ...
      floatingActionButton: FloatingActionButton.extended(
        ...
        label: Text('Agregar Cliente', style: GoogleFonts.dmSans(fontWeight: FontWeight.w600, color: Colors.white)),
      ),
    );
  }

  Widget _buildClienteCard(Map<String, dynamic> cliente, ThemePalette palette) {
    final estadoColor = cliente['estado'] == 'activo' ? const Color(0xFF10B981) : const Color(0xFFEF4444);
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          _buildCreditRow('Límite', cliente['limite'], const Color(0xFF3B82F6)),
          const SizedBox(height: 8),
          _buildCreditRow('Usado', cliente['usado'], cliente['estado'] == 'activo' ? const Color(0xFFF59E0B) : const Color(0xFFEF4444)),
          const SizedBox(height: 8),
          _buildCreditRow('Disponible', cliente['disponible'], const Color(0xFF10B981)),
          const SizedBox(height: 12),
          LinearProgressIndicator(
            value: porcentaje,
            backgroundColor: appThemeNotifier.isDark ? const Color(0xFF262626) : const Color(0xFFE5E7EB),
            valueColor: AlwaysStoppedAnimation<Color>(cliente['estado'] == 'activo' ? const Color(0xFFF59E0B) : const Color(0xFFEF4444)),
          ),
        ],
      ),
    );
  ...
  void _showAddClienteDialog() {
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        title: Text('Agregar Cliente', style: GoogleFonts.syne(fontWeight: FontWeight.w700, color: appThemeNotifier.isDark ? Colors.white : Colors.black)),
        content: Column(
